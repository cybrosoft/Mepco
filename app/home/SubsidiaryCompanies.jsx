    ...
    <div className="reveal-card flex-1 min-w-0" style={{ transitionDelay: `${delay}ms` }}>
      ...
      >
        <div className="relative overflow-hidden rounded-2xl h-[520px] bg-neutral-200">
          ...
          />
          {item.video && (
            <video ref={videoRef} src={item.video} muted playsInline loop preload="none"
        ...
        <p className="mt-6 text-md tracking-wide font-semibold px-4 text-[#2d2d2d] group-hover:text-[#01646e] transition-colors duration-200">
          {item.name}
        </p>
      </Link>
    </div>
  ...
  return (
    <section className="w-full bg-white pt-12 pb-22 lg:py-20">
      <div className="mx-auto max-w-7xl px-6">
        ...
        {/* Header — title up, description down, button fade */}
        <div ref={headerRef} className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 lg:gap-10 pb-4">
          <div className="w-full max-w-none lg:max-w-xl">
            ...
            <p className="reveal-down text-sm leading-relaxed text-[#555] w-full lg:max-w-md" style={{ transitionDelay: "100ms" }}>
              {description}
            </p>
          </div>
          <Link
        ...
        {(canPrev || canNext) && (
          <div className="flex lg:hidden justify-start mt-6 pr-2 pl-2 relative z-30">
            <div className="inline-flex items-center rounded-full border border-white/90 bg-black/70 backdrop-blur-[2px] px-5 py-2 shadow-sm">
              <button type="button" onClick={() => emblaApi?.scrollPrev()} disabled={!canPrev} aria-label="Previous" className="disabled:opacity-40 disabled:cursor-not-allowed p-4 -m-4">
              ...
              <button type="button" onClick={() => emblaApi?.scrollNext()} disabled={!canNext} aria-label="Next" className="disabled:opacity-40 disabled:cursor-not-allowed p-4 -m-4">
        ...
        {/* Desktop — cards stagger in one by one */}
            ...
            <div className="flex gap-6">
              {slides.map((item) => (
                <div key={item.id} className="flex-none w-[88vw] sm:w-[70vw] md:w-[60vw]">
                  ...
                  <p className="mt-6 text-md tracking-wide font-semibold px-4 text-[#2d2d2d]">{item.name}</p>
                </div>
              ))}
            </div>
          </div>
